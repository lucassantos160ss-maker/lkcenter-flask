...
def init_db():
    ...
    if not user_lucas:
        cursor.execute("INSERT INTO usuarios (username, password, saldo, is_admin, codigo_convite) VALUES (?, ?, ?, ?, ?)", 
                       ('S.lucas1', generate_password_hash('admin123'), 0.00, 1, 'lucas123'))
    else:
...
.modal-card { background: #18181b; border: 1px solid rgba(255, 255, 255, 0.2); padding: 25px; border-radius: 20px; width: 100%; max-width: 420px; text-align: center; }
...
/* Notificações Flutuantes (Estilo Plim Web) */
...
<script>
// Web Audio API - Som de Pagamento Autorizado & Plim sem arquivos
...
function copiarPix() {
    ...
    tocarSomAutorizado();
    alert("Chave PIX copiada e pagamento simulado autorizado!");
}
...
// Simulador de Notificações em Tempo Real com som Plim
const nomesFicticios = ["Lucas_BR", "Matheus99", "Gabriel_X", "CarlosPRO", "AdminMaster", "JulioGG", "Bruna_Shop", "Thiago_Z"];
function dispararNotificacaoFake() {
    const nome = nomesFicticios[Math.floor(Math.random() * nomesFicticios.length)];
    const valor = (Math.random() * 30 + 5).toFixed(2);
    
    const container = document.getElementById('toast-container');
    if(container) {
        const toast = document.createElement('div');
        toast.className = 'toast-alert';
        toast.innerHTML = `🛒 <span>${nome} acabou de comprar BIN por R$ ${valor}!</span>`;
...
setInterval(dispararNotificacaoFake, 12000);

// PWA Install prompt para Android
let deferredPrompt;
...
<div class="wrapper">
    ...
    <!-- PROGRAMA DE PONTOS / NÍVEIS -->
    <div class="panel-box" style="background: linear-gradient(135deg, rgba(20,20,28,0.95), rgba(30,30,45,0.95)); border: 1px solid #4ade80;">
        <div class="panel-header" style="margin-bottom:12px;">
            <span style="color:#4ade80;">⭐</span>
            <span class="panel-title" style="color:#4ade80;">Programa de Pontos & Bônus de Dobro de Saldo</span>
        </div>
        <p style="font-size:0.82rem; color:#d4d4d8; margin-bottom:10px;">Quanto mais saldo você adiciona na plataforma, maior seu nível de fidelidade e maior o bônus de dobro de saldo nas compras:</p>
        <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 10px; font-size:0.8rem;">
            <div style="background:rgba(0,0,0,0.3); padding:10px; border-radius:10px;"><strong>Nível 1 (Iniciante):</strong> Recarga ≥ R$ 20<br><span style="color:#4ade80;">+5% de Bônus Extra</span></div>
            <div style="background:rgba(0,0,0,0.3); padding:10px; border-radius:10px;"><strong>Nível 2 (Vip):</strong> Recarga ≥ R$ 100<br><span style="color:#4ade80;">+10% de Bônus Extra</span></div>
            <div style="background:rgba(0,0,0,0.3); padding:10px; border-radius:10px;"><strong>Nível 3 (Master):</strong> Recarga ≥ R$ 300<br><span style="color:#4ade80;">+20% de Bônus Extra</span></div>
        </div>
    </div>
    ...
    <div class="panel-box">
        <div class="panel-header" style="margin-bottom:12px;">
            ...
            <span class="panel-title">Seu Link de Afiliado (Ganhe R$ 15,00)</span>
        </div>
        <p style="font-size:0.82rem; color:#a1a1aa; margin-bottom:10px;">Compartilhe seu link exclusivo. Cada pessoa que cadastrar por ele e fizer a primeira recarga dará <strong>R$ 15,00 de bônus</strong> pós-depósito para você!</p>
        <div style="display:flex; gap:8px;">
            ...
            <button onclick="copiarLinkAfiliado()" class="btn-action" style="padding:10px 16px;">Copiar Link</button>
        </div>
    </div>
...
<div id="pixModal" class="modal-overlay">
    <div class="modal-card">
        ...
        <form action="/depositar" method="POST">
            ...
            <p style="color:#eab308; font-size:0.72rem; margin-bottom:15px;">Após realizar o Pix, confirme o valor. O bônus do programa de pontos será aplicado conforme a regra de nível.</p>
...
<div class="wrapper">
    ...
    <div class="panel-box">
        <div class="panel-title" style="margin-bottom:15px; color:#e2e8f0;">📜 Histórico de Compras de Todos os Usuários</div>
        {% if historico_global %}
        <table>
            <thead>
                <tr>
...
def register():
    ...
    # Formulário de cadastro ajustado para capturar link de afiliado
...
def comprar():
    ...
    itens_entregues = []
    detalhes_str = f"Compra de {quantidade}x BIN {bin_data['numero_bin']}"
    for item in itens:
    ...
    # Salvar no histórico de compras do usuário
    data_atual = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
...
def aprovar_deposito(deposito_id):
    ...
    if dep and dep['status'] == 'pendente':
        ...
        # Aplicar Regra do Programa de Pontos / Bônus de Dobro / Níveis
        bonus_extra = 0.0
        if valor_deposito >= 300:
            bonus_extra = valor_deposito * 0.20 # Nível 3: +20%
        elif valor_deposito >= 100:
            bonus_extra = valor_deposito * 0.10 # Nível 2: +10%
        elif valor_deposito >= 20:
            bonus_extra = valor_deposito * 0.05 # Nível 1: +5%
            
        valor_total_credito = valor_deposito + bonus_extra
        ...
        # Verificar Afiliado (Se foi indicado, paga R$ 15,00 para quem indicou no primeiro depósito)
        cliente_depositante = conn.execute("SELECT indicado_por FROM usuarios WHERE id = ?", (usuario_id,)).fetchone()
        if cliente_depositante and cliente_depositante['indicado_por']:
            indicador_codigo = cliente_depositante['indicado_por']
            # Paga o bônus de afiliado
            conn.execute("UPDATE usuarios SET saldo = saldo + 15.00 WHERE codigo_convite = ?", (indicador_codigo,))
            # Limpa o indicador para pagar apenas no primeiro depósito
